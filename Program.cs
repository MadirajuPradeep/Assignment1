    ...
    class Program
    {
        ...
        */
        ...
        private static string RemoveVowels(string s)
        {
            try
            {
                ...
                {
                    if (!st.Contains(s[i]))  //Checks for vowels in the input string.
                        final_string += s[i]; // Returns the updated string. 
                }
                return final_string; // Returns original string
            }
            catch (Exception)
            {
                throw;
       ...
       */
        ...
        private static bool ArrayStringsAreEqual(string[] bulls_string1, string[] bulls_string2)
        {
            try
            {
                string s1 = bulls_string1.ToString(); // Converting string array to string. 
                string s2 = bulls_string2.ToString(); // Converting string array to string.              
                if (s1 == s2)
                {
                    return true; //true if both strings match
                }

                return false; //false if both strings do not match
            }
            catch (Exception)
            {
        ...
        */
        ...
        private static int SumOfUnique(int[] bull_bucks)
        {
            ...
            {           
                int result = 0;                
                Dictionary<int, int> D = new Dictionary<int, int>(); // Creating a dictionary of type integer, integer.
                for (int i = 0; i < bull_bucks.Length; i++)
                {
                    if (D.ContainsKey(bull_bucks[i]))  //Checking for duplicate value in input array.
                        D[bull_bucks[i]] += 1;
                    else
                        D.Add(bull_bucks[i], 1);
                }

                foreach (var V in D)
                {
                    if (V.Value == 1)  //Filtering the number of unique values.
                        result += V.Key;
                }
                return result;
        ...
        */
        ...
        private static int DiagonalSum(int[,] bulls_grid)
        {
            try
            {
                ...
                for (int i = 0; i < n; i++)
                {
                    sum += bulls_grid[i, i]; //Calculating sum of principle diagonal elements
                    sum += bulls_grid[i, n - i - 1]; //Calculating sum of secondary diagonal elements
                }

                if (n % 2 == 0) //for even sized array, return sum
                    return sum;
                int repeated = bulls_grid[n / 2, n / 2]; //calculating the repeated element for an odd sized array
                return sum - repeated;
            }
            catch (Exception e)
            {
        ...
        */
        ...
        private static string RestoreString(string bulls_string, int[] indices)
        {
            try
            {
                ...
                int n = bulls_string.Length; //Storing the length of the string.            
                char[] Ch = new char[n]; //creating a new char array to store the result string
                if (bulls_string.Length == indices.Length && n > 0 && n <= 100)
                    for (int i = 0; i < n; i++)
                    {
                        Ch[indices[i]] = bulls_string[i];   //rearranging the given string and storing the new result in the char array          
                    }
                return new string(Ch); //passing the final result to the main program
        ...
        */
        ...
        private static string ReversePrefix(string bulls_string6, char ch)
        {
            try
            {
                ...
                {
                    if (bulls_string6[i] == ch) //Checking for occurance of char ch in the input string
                    {
                        ind = i;
                        break;
                    }
                ...
                for (int i = ind; i >= 0; i--)
                {
                    prefix_string += bulls_string6[i]; //Reversing the prefix string
                }
                for (int i = ind + 1; i < bulls_string6.Length; i++)
                {
                    prefix_string += bulls_string6[i]; //Concatinating the prefix string with the remaining part of the string
                }
